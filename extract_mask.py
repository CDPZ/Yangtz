...
def gera_folder_path():
    paths = []
    for i in range(61):  # 文件夹个数110
        if i == 0:
            continue
        elif i < 10:
            path = "E:/final/sub_/Folder 0" + str(i)
            paths.append(path)
        else:
            path = "E:/final/sub_/Folder " + str(i)    #E:/Cgrid_/
            paths.append(path)
    return paths
...
def single(path, year, flag):  # path 这个进程要跑的文件夹(124中的一个）， year本次工作的被裁栅格年份（2002~2015）
    ...
    # raster = "D:\\DaTa\\Yangtz\\workflow\\shape_ehi\\base\\ESACCI-LC-L4-LCCS-Map-300m-P1Y-" + str(year) + "-v2.0.7.tif.tif"  # 被裁的栅格影像所在目录
    raster = "D:/DaTa/Yangtz/workflow/shape_ehi/base/landuse/2010.tif"
    #"D:\DaTa\Yangtz\workflow\shape_ehi\base\landuse\2010.tif"
    ...
    for Inputfeature in shplist_1:
        ...
        if filename[-1] != '_':
            ...
            if int(filename) < int(lastfilename):
                ...
                out = "E:/final/out/" + str(year) + "/" + path[14:] + "/" + filename   # 将shp的名称作为tif输出时的名称    str(year)******************* path[14:
                
                if flag.value == 0:
                    # print("OK")
                    try:
                        if not os.path.isfile(out + ".tif"):
            ...
            if int(filename) < int(lastfilename):
                ...
                out = "E:/final/out/" + str(year) + "/" + path[14:] + "/" + filename + '_'  # 将shp的名称作为tif输出时的名称    str(year)******************* path[14:]
                if flag.value == 0:
                    # print("OK")
                    try:
                        if not os.path.isfile(out + ".tif"):
